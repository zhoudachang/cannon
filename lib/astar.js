...
function moveRange(index, radius) {
    console.log(radius);
    var tempList = [[]];
    tempList[0] = index;
    var rangeList = [[]];
    rangeList[0] = index;
    var countPoint = 0;
    var checkRange = function (node) {
        console.log(node);
        var route = Math.abs(node[0] - index[0]) + Math.abs(node[1] - index[1]);
        if(route <= radius){
            if (!rangeList.find(e => e[0] == node[0] && e[1] == node[1]) && node[0] < 10 && node[1] < 10) {
                rangeList.push(node);
            }
            if(!tempList.find(e => e[0] == node[0] && e[1] == node[1]) && node[0] < 10 && node[1] < 10){
                tempList.push(node);
            }
        }
    }
    var rangeScan = function () {
        for (var i = 0; i < tempList.length; i++) {
            ...
            checkRange([node[0],node[1] + 1]);
            checkRange([node[0],node[1] - 1]);
            checkRange([node[0] + 1,node[1]]);
            checkRange([node[0] - 1,node[1]]);
        }
    };
